...
Completion calls go through `app/llm/`: LangChain chat models (`langchain-google-genai`, `langchain-anthropic`) with `max_retries=0` and provider-default sampling (no explicit temperature — current models recommend or enforce their own defaults), structured output via Pydantic, token/cost logging from `AIMessage.usage_metadata`, and a shared retryable/permanent error taxonomy. Embeddings and the reranker stay on raw `httpx` — they are single vector calls, not workflows.
...
Expected-but-alarming log output (e.g. the Gemini AFC warning on every structured-output call) is catalogued in `KNOWN_LOG_NOISE.md`.
