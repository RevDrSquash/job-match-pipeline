...
| `GET /api/jobs/{id}` | local job detail: same metadata plus `url`, `raw_jd`, and extracted fields (`seniority`, `hard_requirements`, `nice_to_haves`) when present |
...
* `/jobs/{id}` — stored `raw_jd` plus a link out to the original posting.
