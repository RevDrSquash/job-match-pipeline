...
**PoC status:** the schema supports anonymization (`pipeline_events.user_id` is nullable with no FK) but no delete/anonymize path is implemented. Out of scope for the local proof of concept — see `docs/OPEN_ISSUES.md` §10. Required before any real user data.
